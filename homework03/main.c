...
{
	unsigned int * ptr_csr_row_ptr = (unsigned int *) malloc(sizeof(unsigned int) * (m+1));
	unsigned int * ptr_csr_col_ind = (unsigned int *) malloc(sizeof(unsigned int) * nnz);
	double * ptr_csr_vals = (double *) malloc(sizeof(double) * nnz);
	unsigned int* temp_row = (unsigned int*) malloc(sizeof(unsigned int)*m);
	for (int b = 0; b<m; b++){
		temp_row[b] = 0;
	}
	for (int c = 0; c<nnz; c++) {
		temp_row[row_ind[c]-1]++;
	}
	for (int d = 1; d<m; d++){
		temp_row[d] += temp_row[d-1];
	}
	ptr_csr_row_ptr[0] = 0;
	for (int i = 0; i<m; i++){
		ptr_csr_row_ptr[i+1] = temp_row[i];
	}
	unsigned int* temp_row_ptr = (unsigned int *)malloc(sizeof(unsigned int)*(m));
	for (int i = 0; i<m; i++) {
		temp_row_ptr[i] = ptr_csr_row_ptr[i];
	}
	for (int f=0; f<nnz; f++){
		ptr_csr_col_ind[temp_row_ptr[row_ind[f]-1]] = col_ind[f]-1;
		ptr_csr_vals[temp_row_ptr[row_ind[f]-1]] = val[f];
		temp_row_ptr[row_ind[f]-1]++;
	}
	*csr_row_ptr = ptr_csr_row_ptr;
	*csr_col_ind = ptr_csr_col_ind;
	*csr_vals = ptr_csr_vals;
	free(temp_row_ptr);
	free(temp_row);
}
          ...
          double* vector_x, double* res)
{
	for (unsigned int i=0; i<m; i++){
		unsigned int current_row = csr_row_ptr[i];
		unsigned int next_row = csr_row_ptr[i+1];
		double result = 0.0;
		for (int j=current_row; j<next_row; j++){
			int col_index = csr_col_ind[j];
			result += csr_vals[j] * vector_x[col_index];
		}
		res[i] = result;
	}
}
